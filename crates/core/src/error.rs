...
impl HbError {
        ...
        match self {
            HbError::InvalidPassphrase => {
                "Decryption failed. Possible causes:\n\
                 ...
                 Try: Verify you're using the correct passphrase.".into()
            }
            HbError::AuthenticationFailed => {
                "Authentication failed. Possible causes:\n\
                 ...
                 Try: Verify the file integrity and use the correct decryption key.".into()
            }
            HbError::KeyNotFound(fp) => {
                format!("Key '{}' not found in keystore.\n\n\
                        Try:\n\
                        • List available keys with: hb-zayfer keys list\n\
                        • Generate a new key with: hb-zayfer keys generate\n\
                        • Import an existing key with: hb-zayfer keys import", fp)
            }
    ...
    pub fn user_message(&self) -> String {
        match self {
            ...
            HbError::AuthenticationFailed => "File authentication failed - wrong key or corrupted file".into(),
